...
const server=http.createServer((req,res)=>{
  ...
  if(url.pathname==="/health"){res.writeHead(200,{"content-type":"application/json","cache-control":"no-store"});return res.end(JSON.stringify({ok:true,service:"content-hub-assets"}));}
  if(url.pathname==="/manifest.json"){
    const manifest={
      sprite:"/assets/competition-sprite.webp",
  ...
  if(!url.pathname.startsWith("/assets/")){res.writeHead(404);return res.end("Not found");}
